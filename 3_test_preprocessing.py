...
file_link = "/Volumes/Transcend/GitHub/Statistical Machine Learning/Assignment_1/train.txt"
file_link_test = "/Volumes/Transcend/GitHub/Statistical Machine Learning/Assignment_1/test-public.csv"
file_output = "/Volumes/Transcend/GitHub/Statistical Machine Learning/Assignment_1/dataFrame_analysis_test_1.csv"
G = nx.Graph()
...
df_test = pd.read_csv(file_link_test)  
...
df_analysis = pd.DataFrame(columns=['Id', 'Source', 'Sink', 'Jac_Score_Key', 'Jac_Score_Venue', 'neighbors', 'short', 'path_num', 'dif_papers', 'pref', 'adamic', 'sim', 'katz', 'dif_rate', 'dif_last', 'jaccard'])
for index, row in df_test.iterrows():
    
    print(row['Id'])
    keyNode = int(row['Source'])
    keyLink = int(row['Sink'])
    ...
    dfNew = pd.DataFrame({'Source': [keyNode], 'Sink': [keyLink], 'Jac_Score_Key': round(j_score_keyword,6), 'Jac_Score_Venue': round(j_score_venue,6), 'neighbors': numNeighbors_1, 'short': shortest_path, 'path_num': path_num, 'dif_papers': dif_papers, 'pref': pa, 'adamic': adamic, 'resour': ra, 'sim': sim, 'katz':katz_score, 'dif_rate':dif_rate, 'dif_last':dif_last, 'jaccard':jaccard, 'Id': int(row['Id'])})  
    df_analysis = df_analysis.append(dfNew, ignore_index = True)

df_analysis.to_csv(file_output, index = False, header=True)